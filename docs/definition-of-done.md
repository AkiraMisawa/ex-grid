...
| **FN-12e** | MUST | Each `CellTextMetrics` default is at least the widest glyph measured for its class on Windows, macOS and Linux, and the measurements are recorded in ADR-0016; `−`, `+` and `#` are charged in the wide class, and the `####` fill counts `#` at its own width (2026-09-25) | by hand on each platform; Layer 3 on Linux | a bold amount estimated to fit paints without an ellipsis on each platform; `####` itself fits its cell |
...
| **VZ-12a** | MUST | Under a Fill height, the grid takes a definite parent's height: in a sized box and as the rest of a flex column under a toolbar, the Viewport is the parent's content box and P1 holds (ADR-0028, 2026-09-26) | Layer 2 (the inline values) + Layer 3 on a Fill page | rows painted to the box's bottom; the DOM row count is a function of the box, not of the result |
| **VZ-12b** | MUST | A Fill-height grid whose parent has no definite height paints nothing and writes one warning naming the parent (ADR-0028, 2026-09-26) | Layer 2 with a reported band-height; Layer 3 on an auto-height parent | exactly one warning per instance; a real height afterwards paints normally and writes nothing more |
...
| **SR-2a** | MUST | Shift+click on a header selects whole columns from the Anchor's column to the clicked one, and does not sort (ADR-0012, 2026-09-25) | Layer 3 | a whole-column range over the span; no `OnSortChanged` |
...
| **WR-7a** | MUST | Inside `MudExGridPaper` a Fill-height grid takes the paper's height below the toolbar; a declared-height grid lays out as before (ADR-0030, 2026-09-26) | Layer 3 on the MudBlazor pages | the grid's bottom meets the paper's content bottom; the declared grids' boxes are unchanged |
