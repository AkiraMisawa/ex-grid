...
| **ED-17** | MUST | The error message is asked for on demand — at popover open, never per render — and the popover opens after **300 ms of stillness**, on Focus and on hover alike, never permanently (ADR-0034) | Layer 2, counting delegate calls; Layer 3 for the triggers | zero `CellMessageOf` calls while painting; one per open; nothing opens while the Focus or the pointer is still moving |
...
| **ED-21** | MUST | Text typed and not committed that the grid throws away is **announced with its reason** — the order changed under the editor, or the row left the Window (ADR-0011) | Layer 2 for both paths | `OnEditDiscarded` carries `OrderChanged` and `RowLeftTheWindow` respectively; no Edit Intent is raised in either case |
