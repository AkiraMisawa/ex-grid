...
| **FN-12** | MUST | A `Fixed` width below its own `MinWidth` is refused at construction, not clamped; one above `MaxWidth` is the user's and is kept, since `MaxWidth` bounds only what the grid computes (ADR-0016, rewritten 2026-09-25) | Layer 1 + Layer 2 | throws below, naming the column; a drag past `MaxWidth`, recorded as `Fixed(px)` with the default bounds, paints at that width |
