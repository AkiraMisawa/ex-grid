...
| **A11Y-20** | MUST | A **Prerendered** grid carries `aria-busy="true"` on its root, has no `tabindex`, and wears `ex-loading`; its first interactive render lifts all three (ADR-0029/0033) | Layer 2 with the renderer reporting it is not interactive yet; Layer 3 on the Server host, reading the page before its circuit connects | the three present while Prerendered, none of them after |
