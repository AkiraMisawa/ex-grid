...
| **ED-17** | MUST | The error message is asked for on demand — at popover open, never per render — and the popover opens after **300 ms of stillness on the Focus**, never permanently (ADR-0034) | Layer 2, counting delegate calls | zero `CellMessageOf` calls while painting; one per open; a run of arrow keys opens nothing |
...
| **ED-17b** | MUST | The popover **also** opens on hover, after the same 300 ms (ADR-0034) | Layer 3 | **open, and known failing**: a hover that knows its cell needs a pointer-move handler on the Viewport — one interop call per move — which is the per-move cost ADR-0008 refused. Building it is a decision (measure and pay; a cached handler on flagged cells only, against ADR-0003's memoisation; or a fifth JS allowlist entry) and has not been made |
