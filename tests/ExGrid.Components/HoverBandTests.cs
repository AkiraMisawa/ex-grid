...
/// <summary>
/// The hover band (ADR-0029, ADR-0021's fifth entry): the row under the pointer,
/// reported by the browser only when it changes, painted as the Focus band is — one
/// overlay rectangle per layer, no row told. Off by default, and while off the browser
/// is not asked to report at all. 100 columns of 100px, 20px rows in a 100px Viewport.
/// </summary>
public class HoverBandTests : GridTestContext
{
    ...
    private static Task ReportAsync(IRenderedComponent<ExGrid<TestRow>> cut, int row, int column)
        => cut.InvokeAsync(() => cut.Instance.OnPointerCellAsync(row, column));
    ...
    [Fact] // ADR-0021: off by default, the browser is never asked to report the pointer
    public void Off_by_default_the_browser_is_not_asked()
    {
        var cut = RenderGrid(hover: false);

        Assert.DoesNotContain(Js.PointerReporting.Invocations, i => (bool)i.Arguments[0]! == true);
        Assert.Empty(Js.PointerGeometry.Invocations);
    }
    ...
    [Fact] // ADR-0021: on, the browser is told once, with the geometry it computes with
    public void On_the_browser_is_told_once_with_the_geometry()
    {
        var cut = RenderGrid(hover: true);

        Assert.Contains(Js.PointerReporting.Invocations, i => (bool)i.Arguments[0]! == true);
        var geometry = Assert.Single(Js.PointerGeometry.Invocations);
        Assert.Equal(20d, (double)geometry.Arguments[0]!);
        Assert.Equal(101, ((double[])geometry.Arguments[1]!).Length);
        Assert.Equal(0, (int)geometry.Arguments[2]!);
    }

    [Fact] // ADR-0029: the band spans the row, and no row re-renders for it
    ...
    public async Task A_column_change_alone_renders_nothing()
    {
        ...
        await ReportAsync(cut, 2, 4);

        Assert.Equal(renders, cut.RenderCount);
        Assert.Contains("top: 40px", cut.Find(".ex-hover-row").GetAttribute("style"));
    }

    [Fact] // ADR-0029: none means none — the band goes on leave
    ...
    [Fact] // ADR-0021: an index past the rows is none, never a band over empty space
    public async Task An_index_past_the_rows_is_none()
    {
        var cut = RenderGrid(hover: true);

        await ReportAsync(cut, 200, 0);
    ...
    public async Task Turning_it_off_stops_the_report()
    {
        ...
        Assert.Contains(Js.PointerReporting.Invocations, i => (bool)i.Arguments[0]! == false);
        Assert.Empty(cut.FindAll(".ex-hover-row"));
    }
}
